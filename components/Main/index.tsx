...
const Main: React.FC = () => {
  ...
  //   if (ulRef.current && picRef.current) {
  //     const ulBottom = { x: ulRef?.current?.getBoundingClientRect().left, y: ulRef?.current?.getBoundingClientRect().bottom };
  //     const picBottom = { x: picRef?.current?.getBoundingClientRect().left, y: picRef?.current?.getBoundingClientRect().bottom };
      
  //     setUlConnectorPos(ulBottom)
  //     setPicConnectorPos(picBottom)
  //   }
  // })
  ...
  return (
    <div className='h-full w-full relative'>
      ...
      <div className="flex-1 h-full flex flex-col items-center">
        <div className='flex h-full flex-col absolute top-1/4'>
      ...
      <motion.div
        ...
        initial="hidden"
        animate="visible"
        className={`${styles.hexagon} flex justify-center items-center h-full`} style={{height: PICTURE_SIZE + 25, width: PICTURE_SIZE + 25, backgroundColor: theme?.secondary}}>
        <Image
